...
namespace RoleBasedAuthorization.Controllers
{
    public class AccountController : Controller
    {
        ...
        public readonly IAccountInfo  _accountInfo;
        public AccountController(IAccountInfo accountInfo)
        {
        ...
        public async Task<ActionResult> Validate(Admins admin)
        {
            ...
            //var _admin = await _accountInfo.GetAdminInfoByEmail(admin.Email);
            var _admin = await _accountInfo.GetAdminInfoByUsername(admin.Username);
            if (_admin != null)
            {
                if (_admin.Password == admin.Password)
                {
                    ...
                    List<Menus> menus = await _accountInfo.GetMenusByRole(roleId);

                    DataSet ds = new DataSet();
                    ds = ToDataSet(menus);
                    DataTable table = ds.Tables[0];
                    DataRow[] parentMenus = table.Select("ParentId = 0");
                    //DataRow[] parentMenus = table;
        ...
        private string GenerateUL(DataRow[] menu, DataTable table, StringBuilder sb)
        {
            if (menu.Length > 0)
            {
                foreach (DataRow dr in menu)
                {
                    ...
                    //var submenu =  table.Select(String.Format("ParentId = '{0}'", pid));
